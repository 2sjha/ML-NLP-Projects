...
class ChowLiuTree:
    ...
    def learn(self, dataset, r):
        ...
        edgemat[
            edgemat == 0.0
        ] = 1e-20  # to avoid the case where the tree is not connected

        # Randomly set r mutual information scores to 0.
        # Do this only is r is not None
        if r:
            r = int(r * edgemat.shape[0] * edgemat.shape[1])
            rand_r = np.random.randint(0, edgemat.shape[0] * edgemat.shape[1], size=r)
            for flat_idx in rand_r:
                row = flat_idx // edgemat.shape[0]
                col = flat_idx % edgemat.shape[1]
                edgemat[row][col] = 0.0
    ...
    def update(self, dataset, weights):
        ...
        # assume that dataset_.shape[0] equals weights.shape[0]
        # because we assume each example has a weight
        ...
        if weights.shape[0] == dataset.shape[0]:
            ...
            self.xcounts = (
                Util.compute_weighted_xcounts(dataset, weights) + 2.0 * smooth
            )
        else:
        ...
        for i in range(dataset.shape[0]):
            for x in self.topo_order:
                ...
                if self.parents[x] == -9999:
                    ...
                    assigny = dataset[i, y]
                    ll += np.log(
                        self.xyprob[x, y, assignx, assigny] / self.xprob[y, assigny]
                    )
        return ll

    def get_prob(self, sample):
        """
        calculates prob in topological order
        """
        prob = 1.0
        for x in self.topo_order:
            assignx = sample[x]
